...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.Tensor([torch.index_select(
            self.embeddings, 0, w[b]).view(-1).detach().numpy() for b in range(w.shape[0])])
        # END YOUR CODE
        return x
    ...
    def forward(self, w):
        ...
        logits = hidden_activations_droppedout @ self.hidden_to_logits_weight + \
